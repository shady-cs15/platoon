...
def pack_tensor_dict(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in data.items():
        if key == "attention_mask":
            ...
            continue
        # tensors with batch as first dim; align sequence dim to attention mask length, then pack
        if torch.is_tensor(value) and value.ndim >= 2 and value.shape[0] == bs:
            v = value
            if v.shape[1] != seq_len:
                if v.shape[1] < seq_len:
                    pad_width = seq_len - v.shape[1]
                    pad_shape = (bs, pad_width, *v.shape[2:])
                    pad_tensor = torch.zeros(pad_shape, dtype=v.dtype, device=v.device)
                    v = torch.cat([v, pad_tensor], dim=1)
                else:
                    v = v[:, :seq_len, ...]
            packed_tensor = torch.empty(
                (total_length, *v.shape[2:]), dtype=v.dtype, device=v.device
            )
            for i in range(bs):
                start = cu_seqlens[i].item()
                end = cu_seqlens[i + 1].item()
                packed_tensor[start:end] = v[i][: end - start]
...
def split_padded_tensor_dict_into_mb_list(
    ...
    # check tensor shape, split tensors that have batch as first dim
    to_split = {}
    not_to_split = {}
    for key, value in data.items():
        if key == "multi_modal_input":
        ...
        if key == "position_ids" or (
            torch.is_tensor(value) and value.ndim >= 1 and value.shape[0] == bs
        ):

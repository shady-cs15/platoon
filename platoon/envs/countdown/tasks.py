...
class CountDown(object):
    ...
    def get_task(self, id: str) -> Task:
        ...
        return Task(
            goal=f"Using the numbers {nums}, create an equation that equals {target}. You can use basic arithmetic operations (+, -, *, /) and each number can only be used once. Show your work in <think> </think> tags. And return the final answer in <answer> </answer> tags, for example <answer> (1 + 2) / 3 </answer>.",
            max_steps=1,
            misc={"numbers": nums, "target": target},
            id=id,
        )
...
def create_countdown_datasets(
    ...
    for i in tqdm(range(num_samples), desc="Generating training data"):
        task = countdown.get_task(f"countdown.train.{i}")
        train_data.append(task)

    for i in tqdm(range(eval_size), desc="Generating validation/test data"):
        task = countdown.get_task(f"countdown.val.{i}")
...
def get_task_ids(split: Literal["train", "val"], num_samples_train: int = 500000, num_samples_val: int = 1000) -> list[str]:
    if split == "train":
        return [f"countdown.train.{i}" for i in range(num_samples_train)]
    elif split == "val":
        return [f"countdown.val.{i}" for i in range(num_samples_val)]
    else:
        raise ValueError(f"Invalid split: {split}")

def load_task_from_disk(id: str) -> Task:
    if id.startswith("countdown.train."):
        global TRAIN_DATA
        if TRAIN_DATA is None:
            file = pathlib.Path(__file__).parent / f"countdown_train.jsonl"
            TRAIN_DATA = file.read_text().splitlines()
        return Task.from_dict(json.loads(TRAIN_DATA[int(id.split(".")[2])]))
    elif id.startswith("countdown.val."):
        global VAL_DATA
        if VAL_DATA is None:
            file = pathlib.Path(__file__).parent / f"countdown_val.jsonl"
...
if __name__ == "__main__":
    ...
    train_data, val_data = create_countdown_datasets(
    ...
    # Save training data
    train_file = parent_dir / "countdown_train.jsonl"
    ...
    val_file = parent_dir / "countdown_val.jsonl"

from platoon.envs.number_search.tasks import get_task
from platoon.generators.types import RolloutGeneratorConfig
from platoon.envs.number_search.env import NumberSearchEnv
from platoon.agents.number_search.agent import NumberSearchAgent
...
def run_single_rollout_process(args: tuple[str, dict]) -> dict:
    ...
    async def run_rollout() -> dict:
        ...
        try:
            ...
            if hasattr(config, 'llm_client') and config.llm_client is not None:  # type: ignore[attr-defined]
                llm_client = config.llm_client  # type: ignore[attr-defined]
            else:
            ...
            if config.verbose:
                print(
                    f"Process {os.getpid()}: Completed rollout for task {task_id} - "

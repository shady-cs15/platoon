...
from platoon.agents.number_search.rollout import run_single_rollout_process
...
class NumberSearchArealWorkflow:
    ...
    async def arun_episode(self, engine, data: dict) -> dict:
        ...
        config['llm_client'] = client  # Pass Areal client into rollout
        task_id = data['task_id']

        args = (task_id, config)
        # Use spawn to avoid interfering with Areal's event loop

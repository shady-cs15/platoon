...
_current_capture_out: contextvars.ContextVar[Optional[io.TextIOBase]] = \
    contextvars.ContextVar("current_capture_out", default=None)
_current_capture_err: contextvars.ContextVar[Optional[io.TextIOBase]] = \
    contextvars.ContextVar("current_capture_err", default=None)
...
class _Proxy(io.TextIOBase):
    ...
    def write(self, s: str) -> int:
        target = (_current_capture_out.get() if self._which == "out"
                  else _current_capture_err.get())
        (target or self._default).write(s)
        return len(s)

    def flush(self) -> None:
        target = (_current_capture_out.get() if self._which == "out"
                  else _current_capture_err.get()) or self._default
        target.flush()
...
class ShellCapture:
    ...
    def _setup_display_capture(self) -> None:
        ...
        sys.displayhook = CapturingDisplayHook(
            shell=shell, 
            outputs=self._display_buf.outputs
        )

...
class StatsTracker:
    ...
    def _aggregate(self, key: str, entry: StatEntry) -> dict[str, float]:
        ...
        if entry.reduce_type == ReduceType.SCALAR:
            ...
            total = sum(v.sum().item() if isinstance(v, torch.Tensor) else v 
            ...
            mins = [v.min().item() if isinstance(v, torch.Tensor) else v 
                   for v in entry.values]
            if mins:
                result[key] = min(mins)
        
        elif entry.reduce_type == ReduceType.MAX:
            maxs = [v.max().item() if isinstance(v, torch.Tensor) else v 
                   for v in entry.values]
            if maxs:
                result[key] = max(maxs)

...
from platoon.envs.textcraft.tasks import get_task_ids
from datasets import Dataset
from platoon.agents.textcraft.areal_workflow import TextCraftArealWorkflow
...
@dataclass
class TextCraftReinforcePlusPlusConfig(GRPOConfig):
...
def main(args):
    config, _ = load_expr_config(args, TextCraftReinforcePlusPlusConfig)
    config: TextCraftReinforcePlusPlusConfig
    ...
    train_ids = get_task_ids(split="train", num_samples_train=1000)
    ...
    valid_ids = get_task_ids(split="val", num_samples_val=100)
    ...
    workflow = TextCraftArealWorkflow(
        config=config.workflow_config
    )
    eval_workflow = TextCraftArealWorkflow(
        config=config.workflow_config
    )
                ...
                if actor.is_data_parallel_head():
                    ...
                    for data in valid_dataloader:
                        for item in data:
                            ...
                            cnt += 1
                    try:
                        results = eval_rollout.wait(cnt, timeout=1500)
                        ...
                        stats_tracker.get("eval").stat(task_reward=tr, denominator="task_reward_mask")

...
from platoon.envs.number_search.tasks import get_task_ids
from datasets import Dataset
from platoon.agents.number_search.areal_workflow import NumberSearchArealWorkflow
...
@dataclass
class NumberSearchReinforceConfig(GRPOConfig):
...
def main(args):
    config, _ = load_expr_config(args, NumberSearchReinforceConfig)
    config: NumberSearchReinforceConfig
    ...
    workflow = NumberSearchArealWorkflow(
        config=config.workflow_config
    )
    eval_workflow = NumberSearchArealWorkflow(
        config=config.workflow_config
    )
    ...
    for global_step in range(start_step, max_steps):
        ...
        with stats_tracker.record_timing("rollout"):
            ...
            if actor.is_data_parallel_head():
                ...
                stats_tracker.get('reinforce_actor').denominator(task_reward_mask=mask, num_steps_mask=num_steps_mask)
            ...
            def evaluate_fn():
                if actor.is_data_parallel_head():
                    ...
                    try:
                        ...
                        print(f"Num Steps: {results['num_steps']}")
                        
                        tr = torch.as_tensor(results['task_reward'], dtype=torch.float32, device=actor.device)
                        mask = torch.ones_like(tr, dtype=torch.bool)
                        
                        num_steps = torch.as_tensor(results['num_steps'], dtype=torch.float32, device=actor.device)
                        num_steps_mask = torch.ones_like(num_steps, dtype=torch.bool)

                        stats_tracker.get("eval").denominator(task_reward_mask=mask, num_steps_mask=num_steps_mask)
                        stats_tracker.get("eval").stat("task_reward_mask", task_reward=tr)  # default: AVG/MIN/MAX
                        stats_tracker.get("eval").stat("num_steps_mask", num_steps=num_steps)  # default: AVG/MIN/MAX
